...
def _objective(trial: Trial, params: Dict[str, Any], sample: Sample) -> float:
    ...
    model: CatboostRankerModel = CatboostRankerModel(params=params | optimized_params)
    model.train(sample=sample)

    return calculate_topk_percent_auc(model=model, dataset=val)


class CatboostRankerModel(BaseModelTrait, ImplementsRank):
    ...
    def train(self, sample: Sample) -> "CatboostRankerModel":
        self._model = CatBoostRanker(**self.get_params(), verbose=False)
