...
from analysis.utils.columns import COL_TARGET, COL_CURRENCY_PAIR, COL_PUMPED_CURRENCY_PAIR, COL_PUMP_TIME
...
def cross_section_standardisation(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for pump_hash, df_cross_section in tqdm(
            df.groupby("pump_hash"), desc="Applying cross section standardisation"
    ):
