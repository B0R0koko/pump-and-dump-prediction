...
class TOPKPortfolio(ImplementsPortfolio):
    ...
    def __init__(
        ...
        self.impact_bar_minutes: int = impact_bar_minutes
    ...
    def _get_impact_model(self, pump: PumpEvent, cp: CurrencyPair) -> PriceImpactModel:
        ...
        trades_lookback: pd.DataFrame = self.load_trades(bounds=lookback_bounds, currency_pair=cp)
        model = fit_price_impact_model(
            trades=trades_lookback,
            bar_minutes=self.impact_bar_minutes,
            liquidity_quantile=self.impact_liquidity_quantile,
        )
        self._impact_model_cache[cache_key] = model
        return model

    def _apply_price_impact(self, base_price: float, side: int, impact_model: PriceImpactModel) -> tuple[float, float, float]:
        intended_notional_quote: float = max(self.order_notional_quote, 0.0)
        filled_notional_quote: float = impact_model.estimate_fill_notional(
            side=side, intended_notional_quote=intended_notional_quote
        )
        impact_bps: float = impact_model.predict_impact_bps(side=side, notional_quote=filled_notional_quote)
        impacted_price: float = max(1e-12, base_price * (1.0 + side * impact_bps / 1e4))
        return impacted_price, filled_notional_quote, impact_bps
    ...
    def _create_transaction_with_impact(
        ...
        impacted_entry_price, entry_filled_quote, entry_impact_bps = self._apply_price_impact(
            base_price=entry_price, side=1, impact_model=impact_model
        )
        impacted_exit_price, exit_filled_quote, exit_impact_bps = self._apply_price_impact(
            base_price=exit_price, side=-1, impact_model=impact_model
        )
        fill_ratio: float = 0.0
        if self.order_notional_quote > 0:
            fill_ratio = max(0.0, min(entry_filled_quote, exit_filled_quote) / self.order_notional_quote)
        ...
        return Transaction(
            ...
            entry_filled_notional_quote=entry_filled_quote,
            exit_filled_notional_quote=exit_filled_quote,
    ...
    def evaluate_pnl_for_quantities(self, dataset: Dataset, quantities_quote: List[float]) -> pd.DataFrame:
        """
        Re-run backtest for multiple order notionals (quote currency units) with price impact enabled.
        """
        ...
        try:
            ...
            for quantity_quote in quantities_quote:
                ...
                for pump in dataset.get_pumps():
                    ...
                    fill_ratios.extend([tx.fill_ratio for tx in stats.txs if not tx.is_empty()])
...
def evaluate_topk_pnl_for_quantities(
    ...
    manager = TOPKPortfolio(
        ...
        impact_bar_minutes=impact_bar_minutes,
        impact_liquidity_quantile=impact_liquidity_quantile,
    )
    return manager.evaluate_pnl_for_quantities(dataset=dataset, quantities_quote=quantities_quote)
