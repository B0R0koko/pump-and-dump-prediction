...
def calculate_topk(df: pd.DataFrame, probas_pred: np.ndarray, bins: Iterable[int]) -> pd.Series:
    """
    :param df: DataFrame created by create_dataset function from analysis.utils.build_dataset
    :param probas_pred: predicted probabilities of the positive label
    ...
    of size K
    """
    _df = df.copy()  # create a copy such that we don't mutate the initial data
    ...
    _df.drop(columns=[COl_PROBAS_PRED], inplace=True)

    num_pumped: int = df[df[COL_TARGET] == True].shape[0]
    counts = np.array(list(count_by_bins.values()))

    return pd.Series(data=counts / num_pumped, index=bins)


def calculate_topk_percent(df: pd.DataFrame, probas_pred: np.ndarray, bins: Iterable[float]) -> pd.Series:
    """
    :param df: DataFrame created by create_dataset function from analysis.utils.build_dataset
    :param probas_pred: predicted probabilities of the positive label
...
def calculate_topk_percent_auc(df: pd.DataFrame, probas_pred: np.ndarray) -> float:
    ...
    topk_percentages: pd.Series = calculate_topk_percent(
        df=df, probas_pred=probas_pred, bins=np.arange(0, 1.01, 0.005)
    )
