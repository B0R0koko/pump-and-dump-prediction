...
def test_compute_features_matches_feature_definitions(monkeypatch) -> None:
    ...
    writer: PumpsFeatureWriter = _build_writer(
        pump_events=[
            PumpEvent(currency_pair=currency_pair, time=datetime(2020, 1, 1, 0, 0, 0), exchange=Exchange.BINANCE_SPOT),
            PumpEvent(currency_pair=other_pair, time=datetime(2020, 3, 1, 0, 0, 0), exchange=Exchange.BINANCE_SPOT),
            PumpEvent(currency_pair=currency_pair, time=datetime(2020, 6, 1, 0, 0, 0), exchange=Exchange.BINANCE_SPOT),
            PumpEvent(currency_pair=currency_pair, time=datetime(2021, 2, 1, 0, 0, 0), exchange=Exchange.BINANCE_SPOT),
            pump_event,
        ]
    )
    ...
    assert np.isclose(features[FeatureType.ASSET_RETURN_ZSCORE.col_name(window)], expected_asset_return_zscore)
    assert np.isclose(features[FeatureType.QUOTE_ABS_ZSCORE.col_name(window)], expected_quote_abs_zscore)
    ...
    assert np.isclose(features[FeatureType.SLIPPAGE_IMBALANCE.col_name(window)], expected_slippage_imbalance)
